...
;ORIG    (control-lock-map-key s (downcase s) s)
    (setq c (+ c 1))))
...
(let ((c ?0) s)
  (while (<= c ?9)
    (setq s (char-to-string c))
    (control-lock-map-key s (concat "M-" s) s)
;ORIG    (control-lock-map-key s (concat "C-" s) s)
    (setq c (+ c 1))))
...
;ORIG (defun control-lock-keys ()
;ORIG   "Sets default keys - C-z enables control lock."
;ORIG   (global-set-key (kbd "C-z") 'control-lock-enable)
;ORIG   (global-set-key (kbd "C-,") 'control-lock-enable)
;ORIG )

(defun control-lock-global-keys ()
  "Sets default keys - C-u enables control lock, C-i disables control lock."
  (global-unset-key (kbd "C-u"))
  (global-set-key (kbd "C-u") 'control-lock-enable)
  (global-unset-key (kbd "C-i"))
  (global-set-key (kbd "C-i") 'control-lock-disable)
...
(defun control-lock-disable () (interactive)
  "Disables control lock.  This function should be mapped to the key the
user uses to enable control-lock."
  (if control-lock-mode-p
    (progn
      (setq control-lock-mode-p nil)
